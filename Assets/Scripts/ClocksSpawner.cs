...
public class ClocksSpawner : MonoBehaviour
{
    ...
    [SerializeField] private int maxNumPerLevel = 20;
    [SerializeField] private int totalNumOfClocks = 50;
    ...
    void Start()
    {
		...
		_spawnTimer.AddTimerFinishedEventListener(HandleSpawnTimerFinishedEvent);
        _spawnTimer.Duration = SPAWN_DELAY;
        _spawnTimer.Run();
    ...
    {		
		...
		{
			var clockToSpawnIndex = Random.Range(0, clockPrefabsRef.Length);
            GameObject pickup = Instantiate(clockPrefabsRef[clockToSpawnIndex],
